...
RUN wget --no-check-certificate -qO- https://github.com/DFHack/dfhack/releases/download/0.40.24-r5/dfhack-0.40.24-r5-Linux-gcc-4.5.4.tar.bz2 | tar -xj -C /app/df_linux && rm -rf df_linux/hack/plugins/*
...
ADD init.txt /app/df_linux/data/init/init.txt
ADD dfhack.init /app/df_linux/dfhack.init

WORKDIR /app/df_linux

EXPOSE 1235/udp

RUN mkdir /app/df_linux/data/save
VOLUME /app/df_linux/data/save

CMD /app/df_linux/dfhack
